...
- **`aps_mcp.py`** — 51 tools for navigation, folder/file operations, permission auditing, bulk user management, hub-level directory onboarding, bulk folder reorg, ACC Issues (list/get/create/update + types & custom-field metadata), and ACC approval workflows (list/get/create + bulk-create)
...
All 51 tools are registered via `@app.call_tool`. The key layers:
...
- **Approval-workflow tools**: `list_workflows`, `get_workflow`, `create_workflow`, `bulk_create_workflows` wrap the ACC Reviews `construction/reviews/v1/projects/{bareProjectId}/workflows` API (user-scoped 3-legged). Like Issues they send a constant `x-ads-region: {REVIEWS_REGION}` header (`REVIEWS_REGION = "EMEA"`, module-level) and reuse `_get_all_issues()` — the Workflows list envelope is the same top-level `pagination.totalResults` shape (capped at `limit=50`, so `page_limit=50`). `_resolve_review_project()` mirrors `_resolve_issue_project()` (bare UUID); `_resolve_workflow_ref()` resolves a workflow by `workflow_id` or by `name` (one list call, exact case-insensitive match). **Reviewer/candidate resolution is the crux, and involves two distinct ID spaces**: the Reviews API keys every step candidate by `autodeskId`. For **users**, that is the opaque `autodeskId` from the admin `projects/{id}/users` payload (now surfaced as `autodesk_id` by `list_project_members`/`list_project_roles`). For **roles and companies**, the Reviews API uses a *separate numeric* `autodeskId` that is **not** the project-role UUID from the Admin API, and Autodesk exposes no endpoint to discover it (confirmed live: a project role's Admin-API `id` is a UUID like `xxxxxxxx-xxxx-…`, whereas the same role's `autodeskId` in a workflow's candidates is a short *numeric* string — two different values). So `_build_project_directory()` resolves users from project members but **harvests** role/company name→autodeskId from the candidates already embedded in the project's **existing** workflows (a `list_workflows` sweep) — the only source in the right ID space. `_resolve_candidates()` translates a step's `reviewer_users`/`reviewer_roles`/`reviewer_companies` into `{users/roles/companies:[{autodeskId}]}`, passing through anything that already looks like a raw id (`_looks_like_autodesk_id()`) and raising loudly on any unresolved name (a role/company unused by any existing workflow must be given as a raw numeric id from `get_workflow`). `_workflow_create_payload()` (shared by the single and bulk create) builds the POST body from only supplied keys (snake_case → API camelCase), defaulting the API-required `copyFilesOptions` to `{enabled:false}`. `create_workflow` **posts immediately (no `dry_run`, like `create_issue`)**; `bulk_create_workflows` is the Excel-batch tool — it resolves the directory **once**, defaults to `dry_run=True` (`would_create`), fans the POSTs out via `_gather_bounded()`, maps a 409 name collision to `already_exists` per row, writes a timestamped audit CSV live, and shapes output via `_shape_bulk_response()` (noteworthy = error/already_exists). Scope is list/get/create only — the Reviews API exposes no workflow update/delete route.
...
`main` is protected on GitHub (changes go through a **pull request** with a passing **`test`** CI check). Never commit directly to `main`. Follow this every time:

1. **Sync first, then branch.** At the start of a piece of work: `git checkout main && git pull --ff-only`, then `git checkout -b <feat|fix|docs>/<short-name>`. Branch off a *fresh* `main` so the branch never starts stale. (If coding started on `main` by accident, just branch **before the first commit** — uncommitted changes follow you across the checkout. The rule is *branch before you commit*, not before you code.)
2. **One branch per logical unit** of work — not per tiny edit. Trivial doc tweaks can wait and ride along in the next feature branch.
3. **Land via PR.** Push the branch (`git push -u origin <branch>`), open a PR, let CI's `test` check run, then merge. Prefer a **fast-forward / linear history** (`git merge --ff-only` locally; if `main` moved, `git fetch && git rebase origin/main` on the branch first). No merge commits.
4. **`gh` CLI is not installed** (see the no-gh memory) — open PRs via the GitHub web UI, or with `gh pr create` once it's available. Plain `git push` needs no `gh` (credentials are cached by Git Credential Manager).
5. Only commit/push when the user asks. Run the Sensitive Data Check below before staging.
